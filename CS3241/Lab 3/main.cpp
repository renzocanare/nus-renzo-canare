...
const char creeperTexFile[] = "images/creeper.jpg"; // For Task 2.
const char creeperFaceTexFile[] = "images/creeper_face.jpg"; // For Task 2.
...
GLuint spotsTexObj;
GLuint creeperTexObj; // For Task 2.
GLuint creeperFaceTexObj; // For Task 2.
...
void DrawTable( void );
void DrawCreeperBody( void ); // For Task 2.
void DrawCreeperFace( void ); // For Task 2. 
void DrawCreeper( void ); // For Task 2.
...
void MakeReflectionImage( void )
{
    ...
    // STEP 2: Sets up the correct view volume for the imaginary viewpoint.
    double eyePosZ = 2.0 * TABLETOP_Z - eyePos[2];

    // STEP 3: Sets up the imaginary viewpoint.
    double frustLeft = TABLETOP_Y1 - eyePos[1];
    double frustRight = TABLETOP_Y2 - eyePos[1];
    double frustBottom = TABLETOP_X1 - eyePos[0];
    double frustTop = TABLETOP_X2 - eyePos[0];
    double frustNear = eyePos[2] - TABLETOP_Z;
    double frustFar = ROOM_HEIGHT * 10.0;
    ...
    glLoadIdentity();
    gluLookAt( eyePos[0], eyePos[1], eyePosZ, eyePos[0], eyePos[1], eyePos[2], 1.0, 0.0, 0.0 );
...
// This texture object is for storing the reflection image read from the color buffer.
    ...
    // Repeat Wraps S and T, set magnifying and minifying functions to linear, set automatic mipmap generation.
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR );
    ...
    // TASK 2: This texture object is for the creeper texture map.
    glGenTextures( 1, &creeperTexObj );
    glBindTexture( GL_TEXTURE_2D, creeperTexObj );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR );

    if ( ReadImageFile( creeperTexFile, &imageData,
        &imageWidth, &imageHeight, &numComponents ) == 0 ) exit( 1 );
    if ( numComponents != 3 )
    {
        fprintf(stderr, "Error: Texture image is not in RGB format.\n");
    ...
    // TASK 2: This texture object is for the creeper face texture map.
    glGenTextures( 1, &creeperFaceTexObj );
    glBindTexture( GL_TEXTURE_2D, creeperFaceTexObj );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR );

    if ( ReadImageFile( creeperFaceTexFile, &imageData,
        &imageWidth, &imageHeight, &numComponents ) == 0 ) exit( 1 );
    if ( numComponents != 3 )
    {
        fprintf(stderr, "Error: Texture image is not in RGB format.\n");
...
// Tabletop.
    ...
    // 
    // void SubdivideAndDrawQuad( int uSteps, int vSteps, float s0, float t0, float x0, float y0, float z0,
    //                                                    float s1, float t1, float x1, float y1, float z1,
    //                                                    float s2, float t2, float x2, float y2, float z2,
    //                                                    float s3, float t3, float x3, float y3, float z3 )
    //********************************************************
...
// TASK 2: Draws one leg of the Creeper, front left by default.
void DrawOneCreeperLeg( void )
{
    glBegin( GL_QUADS );
        // Draw top
        glTexCoord2d( 1.0, 1.0 ); glVertex3d( 1.0, -1.3, -0.9 );
        glTexCoord2d( 0.0, 1.0 ); glVertex3d( 0.4, -1.3, -0.9 );
        glTexCoord2d( 0.0, 0.0 ); glVertex3d( 0.4, -1.3, -0.3 );
        glTexCoord2d( 1.0, 0.0 ); glVertex3d( 1.0, -1.3, -0.3 );
        // Draw bottom
        glTexCoord2d( 1.0, 1.0 ); glVertex3d( 1.0, -2.0, -0.3 );
        glTexCoord2d( 0.0, 1.0 ); glVertex3d( 0.4, -2.0, -0.3 );
        glTexCoord2d( 0.0, 0.0 ); glVertex3d( 0.4, -2.0, -0.9 );
        glTexCoord2d( 1.0, 0.0 ); glVertex3d( 1.0, -2.0, -0.9 );
        // Draw front
        glTexCoord2d( 1.0, 1.0 ); glVertex3d( 1.0, -1.3, -0.3 );
        glTexCoord2d( 0.0, 1.0 ); glVertex3d( 0.4, -1.3, -0.3 );
        glTexCoord2d( 0.0, 0.0 ); glVertex3d( 0.4, -2.0, -0.3 );
        glTexCoord2d( 1.0, 0.0 ); glVertex3d( 1.0, -2.0, -0.3 );
        // Draw back
        glTexCoord2d( 1.0, 1.0 ); glVertex3d( 1.0, -2.0, -0.9 );
        glTexCoord2d( 0.0, 1.0 ); glVertex3d( 0.4, -2.0, -0.9 );
        glTexCoord2d( 0.0, 0.0 ); glVertex3d( 0.4, -1.3, -0.9 );
        glTexCoord2d( 1.0, 0.0 ); glVertex3d( 1.0, -1.3, -0.9 );
        // Draw left
        glTexCoord2d( 1.0, 1.0 ); glVertex3d( 0.4, -1.3, -0.3 );
        glTexCoord2d( 0.0, 1.0 ); glVertex3d( 0.4, -1.3, -0.9 );
        glTexCoord2d( 0.0, 0.0 ); glVertex3d( 0.4, -2.0, -0.9 );
        glTexCoord2d( 1.0, 0.0 ); glVertex3d( 0.4, -2.0, -0.3 );
        // Draw right
        glTexCoord2d( 1.0, 1.0 ); glVertex3d( 1.0, -1.3, -0.9 );
        glTexCoord2d( 0.0, 1.0 ); glVertex3d( 1.0, -1.3, -0.3 );
        glTexCoord2d( 0.0, 0.0 ); glVertex3d( 1.0, -2.0, -0.3 );
        glTexCoord2d( 1.0, 0.0 ); glVertex3d( 1.0, -2.0, -0.9 );
    glEnd();
}

// TASK 2: Draws the face of the Creeper, applies face texture to Creeper.
void DrawCreeperFace(void)
{
    ...
    glPushMatrix();
        glTranslated(0.7, -1.3, TABLETOP_Z + TABLETOP_Z / 2.0);
        glRotated(90.0, 1.0, 0.0, 0.0);
        glScaled(0.25, 0.25, 0.25);
        glBegin(GL_QUADS);
            glTexCoord2d( 1.0, 1.0 ); glVertex3d( 0.5, 3.0, -1.0 );
            glTexCoord2d( 0.0, 1.0 ); glVertex3d( 0.5, 3.0, 0.5 );
            glTexCoord2d( 0.0, 0.0 ); glVertex3d( 0.5, 1.5, 0.5 );
            glTexCoord2d( 1.0, 0.0 ); glVertex3d( 0.5, 1.5, -1.0 );
        glEnd();
    glPopMatrix();
}

// TASK 2: Draws the body of the Creeper, applies body texture to Creeper.
void DrawCreeperBody( void )
{
    GLfloat matAmbient[] = { 0.7, 0.7, 0.7, 1.0 };
    GLfloat matDiffuse[] = { 0.7, 0.7, 0.7, 1.0 };
    GLfloat matSpecular[] = { 1.0, 1.0, 1.0, 1.0 };
    ...
    glPushMatrix();
        ...
        glBegin( GL_QUADS );
            // Draw top
            glTexCoord2d( 1.0, 1.0 ); glVertex3d( 0.5, 3.0, -1.0 );
            glTexCoord2d( 0.0, 1.0 ); glVertex3d( -1.0, 3.0, -1.0 );
            glTexCoord2d( 0.0, 0.0 ); glVertex3d( -1.0, 3.0, 0.5 );
            glTexCoord2d( 1.0, 0.0 ); glVertex3d( 0.5, 3.0, 0.5 );
            // Draw bottom
            glTexCoord2d( 1.0, 1.0 ); glVertex3d( 0.5, 1.5, 0.5 );
            glTexCoord2d( 0.0, 1.0 ); glVertex3d( -1.0, 1.5, 0.5 );
            glTexCoord2d( 0.0, 0.0 ); glVertex3d( -1.0, 1.5, -1.0 );
            glTexCoord2d( 1.0, 0.0 ); glVertex3d( 0.5, 1.5, -1.0 );
            // Draw front
            glTexCoord2d( 1.0, 1.0 ); glVertex3d( 0.5, 3.0, 0.5 );
            glTexCoord2d( 0.0, 1.0 ); glVertex3d( -1.0, 3.0, 0.5 );
            glTexCoord2d( 0.0, 0.0 ); glVertex3d( -1.0, 1.5, 0.5 );
            glTexCoord2d( 1.0, 0.0 ); glVertex3d( 0.5, 1.5, 0.5 );
            // Draw back
            glTexCoord2d( 1.0, 1.0 ); glVertex3d( 0.5, 1.5, -1.0 );
            glTexCoord2d( 0.0, 1.0 ); glVertex3d( -1.0, 1.5, -1.0 );
            glTexCoord2d( 0.0, 0.0 ); glVertex3d( -1.0, 3.0, -1.0 );
            glTexCoord2d( 1.0, 0.0 ); glVertex3d( 0.5, 3.0, -1.0 );
            // Draw left
            glTexCoord2d( 1.0, 1.0 ); glVertex3d( -1.0, 3.0, 0.5 );
            glTexCoord2d( 0.0, 1.0 ); glVertex3d( -1.0, 3.0, -1.0 );
            glTexCoord2d( 0.0, 0.0 ); glVertex3d( -1.0, 1.5, -1.0 );
            glTexCoord2d( 1.0, 0.0 ); glVertex3d( -1.0, 1.5, 0.5 );
        glEnd();

        // Draw Body
        glBegin( GL_QUADS );
            // Draw top
            glTexCoord2d( 1.0, 1.0 ); glVertex3d( 0.4, 1.5, -0.9 );
            glTexCoord2d( 0.0, 1.0 ); glVertex3d( -0.9, 1.5, -0.9 );
            glTexCoord2d( 0.0, 0.0 ); glVertex3d( -0.9, 1.5, 0.4 );
            glTexCoord2d( 1.0, 0.0 ); glVertex3d( 0.4, 1.5, 0.4 );
            // Draw bottom
            glTexCoord2d( 1.0, 1.0 ); glVertex3d( 0.4, -1.5, 0.4 );
            glTexCoord2d( 0.0, 1.0 ); glVertex3d( -0.9, -1.5, 0.4 );
            glTexCoord2d( 0.0, 0.0 ); glVertex3d( -0.9, -1.5, -0.9 );
            glTexCoord2d( 1.0, 0.0 ); glVertex3d( 0.4, -1.5, -0.9 );
            // Draw front
            glTexCoord2d( 1.0, 1.0 ); glVertex3d( 0.4, 1.5, 0.4 );
            glTexCoord2d( 0.0, 1.0 ); glVertex3d( -0.9, 1.5, 0.4 );
            glTexCoord2d( 0.0, 0.0 ); glVertex3d( -0.9, -1.5, 0.4 );
            ...
            glTexCoord2d( 1.0, 0.0 ); glVertex3d( 0.4, 1.5, -0.9 );
            // Draw left
            glTexCoord2d( 1.0, 1.0 ); glVertex3d( -0.9, 1.5, 0.4 );
            glTexCoord2d( 0.0, 1.0 ); glVertex3d( -0.9, 1.5, -0.9 );
            glTexCoord2d( 0.0, 0.0 ); glVertex3d( -0.9, -1.5, -0.9 );
            glTexCoord2d( 1.0, 0.0 ); glVertex3d( -0.9, -1.5, 0.4 );
            // Draw right
            glTexCoord2d( 1.0, 1.0 ); glVertex3d( 0.4, 1.5, -0.9 );
            glTexCoord2d( 0.0, 1.0 ); glVertex3d( 0.4, 1.5, 0.4 );
            glTexCoord2d( 0.0, 0.0 ); glVertex3d( 0.4, -1.5, 0.4 );
            glTexCoord2d( 1.0, 0.0 ); glVertex3d( 0.4, -1.5, -0.9 );
        glEnd();
...
// TASK 2: Draws whole creeper.
void DrawCreeper(void)
{
    DrawCreeperFace();
    DrawCreeperBody();
}
