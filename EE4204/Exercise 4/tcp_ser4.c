...
void str_ser(int sockfd)
{
	...
	while(!end)
	{
		...
		// START // When window size is reached, send ACK to acknowledge
		// complete transfer and then reset window count to 0.

		if (du_received == WINDOWSIZE || end == 1 )
		{
			ack.num = 1;
			ack.len = 0;

			n = send(sockfd, &ack, 2, 0);						//send the ack
	...
	fclose(fp);
	printf("Number of ACKs sent out: %d \n", ack_sentout);
	ack_sentout = 0;
