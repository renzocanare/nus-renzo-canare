  ...
  /*** the whole file is loaded in the buffer. ***/
	...
	while(ci<= lsize)
	{
		if ((lsize+1-ci) <= DATALEN)
			...
			slen = DATALEN;									//sending file in data units of size DATALEN
		...
		// START // When amount of DU sent out is equal to window size, wait for
		// ack from server.

		du_sentout++;

		if (du_sentout == WINDOWSIZE)
		{
			n = recv(sockfd, &ack, 2, 0);							// client waits for the ack
